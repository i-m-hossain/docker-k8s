## creating secret object with imperative command
...
## kubernetes volume vs Persistent volume vs Persistent claim volume:

**kubernetes volume**:
Kubernetes Volume is tied to the pod. when the pod is created, the volume is created. if container dies, volume still persists. But in case the pod dies the volume also dies.

**Persistent volume**:
Persistent volume still persists, if the pod dies. So it is very handy for database pods. 
- statically provisioned persistent volume
- dynamically provisioned persistent volume
**Persistent Volume Claim**:
kind of like configuration to claim persistent volume
...
## Accessing minikube docker server from docker cli:

```
eval $(minikube docker-env)
```

## Imperative updating a deployment's image:

```
kubectl set image <object_type>/<object_name> <container_name>=<new image to use>
kubectl set image deployment/client-deployment client=stephengrider/multiclient:v5
```

## Pods vs Deployments:

Pods:
i. Runs a single set of containers
ii. Good for one-off dev purposes
iii. Rarely used directly in production

Deployments:
i. Runs a set of identical pods(one or more)
ii. Monitors the state of each pod, updating as necessary
iii. Good for dev
iv. Good for production

## some important takeaways

i. Kubernetes is a system to deploy containerized apps
ii. Nodes are individual machines(or vm's) that run containers
iii. Masters are machines(or vm's) with a set of programs to manage nodes
iv. Kubernetes didn't build our images -it got them from somewhere else i.e. dockerhub
v. Kubernetes(the master) decided where to run each container - each node can run a dissimilar set of containers.
vi. To deploy something, we update the desired state of the master with a config file
vii. The master works constantly to meet your desired state
...
-   minikube start: creates a virtual machine as a node
-   Pods: Runs one or more closely related containers
-   Services: Sets up networking in a Kubernetes Cluster
    -   ClusterIp - Exposes a set of pods to other objects in the cluster
    -   NodePort - exposes a container to the outside world(only good for dev purposes!) (30000-32767)
    -   Ingres
    -   LoadBalancer
-   kube-proxy(routes the request to different services)

## What config files do:

Config File ----(used to create 'Objects')---> StatefulSet || ReplicaSet || Pod || Service || Namespace || Event || Endpoints || configMap || ComponentStatus || Deployments || Volumes

-   Objects serve different purposes including: running a container, monitoring a container, setting up networking, etc.
-   Pod is used to run a container
-   Service is used to set up networking
...
2. compose- Each entry represents a container we want to create
3. compose - Each entry defines the networking requirements

**Kubernetes**

1. Kubernetes expects all images to already be built
2. One config file per object we want to create
3. k8s - We have to manually set up all networking

to transform compose to kubernetes:
...
_kubectl_- used for managing containers in the (local + prod)
_minikube_- used for managing the VM itself (local)

## Kubernetes what and why:

1. **What**: System for running many different containers over multiple different machines
2. **Why**: When you need to run many different containers with different images
